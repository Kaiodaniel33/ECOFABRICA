document.addEventListener("DOMContentLoaded", () => {
    ...
    const SABIA_KNOWLEDGE_BASE = {
        gameTips: [
            ...
            { condition: (s) => s.marketPriceModifier > 1.3, tip: `O mercado está pagando ${((s.marketPriceModifier - 1) * 100).toFixed(0)}% a mais! Excelente hora para focar em 'Produção'.` },
            { condition: (s) => s.marketPriceModifier < 0.9, tip: "O mercado está em baixa... Estamos recebendo menos pelos produtos. Talvez seja hora de focar em cortar custos e desperdício." },
    ...
    class GameState {
        ...
        get() {
            return {
                ...this.currentState, playerName: this.playerName,
                managerProfile: this.managerProfile, currentMonth: this.currentMonth,
                unlockedTech: this.unlockedTech, unlockedActions: this.unlockedActions, unlockedFlags: this.unlockedFlags
            };
        }
        ...
        initialize(playerName, profileId) {
            ...
            this.currentState = { ...GAME_DATA.initialState };
            this.currentMonth = 0; this.isEventActive = false;
            this.unlockedTech = []; this.unlockedActions = []; this.unlockedFlags = {};
            this.triggeredEvents = [];
            this.monthlyEffects = { budget_income: 0, budget_expense: 0, marketPriceModifier: 0.0, production: 0, morale: 0, waste: 0 };
            this.history = { budget: [], sustainability: [], waste: [] };
        ...
        processMonthlyTurn() {
            ...
            s.morale += (Math.random() * 4 - 2); s.reputation -= 1;
            this.clampStats();
        ...
        checkForRandomEvent() {
            const s = this.get();
            const potentialEvents = GAME_DATA.events.filter(event => event.condition(s) && !this.triggeredEvents.includes(event.id));
        ...
        getFinalMessage(isBankruptcy) {
            if (isBankruptcy) { return `Que pena... A ECOFÁBRICA faliu no Mês ${this.currentMonth}. Mas não desanime, cada jornada tem seus tropeços. Tente novamente!`; }
            ...
            else { return `A gestão foi um grande aprendizado, ${this.playerName}. Você não atingiu a meta de ${profile.name}, mas cada desafio ensina. Tente novamente com outra estratégia!`; }
        }
    }
    ...
    class GameUI {
        ...
        bindEvents(gameInstance) {
            // Pre-select first profile & enable button
            ...
            this.dom.decisionsColumn.addEventListener('click', (e) => {
                ...
                if (button && !button.disabled) { gameInstance.handleAction(button.dataset.actionId); }
            });
            this.dom.researchColumn.addEventListener('click', (e) => {
                const button = e.target.closest('button[data-research-id]');
                if (button && !button.disabled) { gameInstance.handleResearch(button.dataset.researchId); }
            });

            this.dom.closeModalButtons.forEach(btn => {
                btn.addEventListener('click', () => {
                    const modalId = btn.dataset.modalId;
                    if (modalId) { this.showModal(modalId, false); }
                });
            });
            ...
            window.addEventListener('click', (event) => {
                if (event.target.classList.contains('c-modal')) {
                    ...
                    if (modalId !== 'event-modal') { this.showModal(modalId, false); }
                }
            });
        ...
        switchScreen(screenName) {
            ...
            if (screenName === 'game') { this.dom.gameScreen.classList.remove('u-hidden'); }
            else if (screenName === 'end') { this.dom.endScreen.classList.remove('u-hidden'); }
            else { this.dom.startScreen.classList.remove('u-hidden'); }
        }

        toggleMusic(forcePlay = false) {
            const audio = this.dom.music.audio;
            const btn = this.dom.music.toggleBtn;
        ...
        render(state, history, isEventActive) {
            // Header
            this.dom.gameHeader.textContent = `${state.playerName} · Mês ${state.currentMonth} / ${GAME_DATA.totalMonths}`;

            // Progress bar
            const pct = (state.currentMonth / GAME_DATA.totalMonths) * 100;
            if (this.dom.monthProgressBar) { this.dom.monthProgressBar.style.width = `${pct}%`; }
            if (this.dom.monthDisplay) { this.dom.monthDisplay.textContent = `${state.currentMonth}/${GAME_DATA.totalMonths}`; }

            // RP counter
            if (this.dom.rpCounter) { this.dom.rpCounter.textContent = `${state.researchPoints} P&D`; }
        ...
        renderStats(state, targetElement) {
            ...
            Object.keys(displayNames).forEach(key => {
                ...
                const name = displayNames[key]; const value = state[key];
                let valueStr = '';
                ...
                else if (key.match(/sustainability|morale|reputation/)) { valueStr = `${value.toFixed(0)}/100`; }
                else { valueStr = value.toFixed(0); }

                const item = document.createElement('div'); item.className = 'c-stats-grid__item';
                item.innerHTML = `<span class="c-stats-grid__label">${name}</span><span class="c-stats-grid__value ${modifierClass}">${valueStr}</span>`;
        ...
        renderActions(state) {
            ...
            categoryOrder.forEach(catName => {
                if (categories[catName]) {
                    const h4 = document.createElement('h4'); h4.textContent = catName;
        ...
        renderResearch(state) {
            this.dom.researchColumn.innerHTML = '';
        ...
        createActionButton(item, type, state) {
            ...
            const cost = item.cost || 0; const costRP = item.costRP || 0;
            let reason = '';
            ...
            button.innerHTML = `<strong>${item.label}</strong><small>${item.description}<br>Custo: ${costStr}${monthlyCostStr}</small>`;
            return button;
        }
        ...
        updateButtonStates(state, isEventActive) {
            ...
            if (isEventActive) {
                this.dom.decisionsColumn.querySelectorAll('button').forEach(b => { b.disabled = true; });
                this.dom.researchColumn.querySelectorAll('button').forEach(b => { b.disabled = true; });
            }
        }
        ...
        showEventModal(event, choiceCallback) {
            ...
            event.choices.forEach((choice, index) => {
                ...
                button.onclick = () => { choiceCallback(index); this.showModal('eventModal', false); };
        ...
        renderReportChart(history) {
            ...
            if (history.budget.length === 0) {
                this.dom.chartContainer.innerHTML = "<p style='color:var(--text-3);font-size:.875rem;padding:1rem;'>Nenhum dado ainda. Avance pelo menos um mês.</p>";
                return;
            }
        ...
        renderEndScreen(state, finalMessage, isWin) {
            ...
            this.dom.finalMessage.className = 'final-message';
    ...
    class ProfessorSabia {
        ...
        log(message, type = 'sabia') {
            this.ui.addLog(`<strong>Professor Sabiá:</strong> "${message}"`, type);
        }

        greet(playerName, profileName) {
            this.log(`Olá, ${playerName}! Um ${profileName}, hein? Boa escolha! Sua jornada na EcoFábrica começa agora. Estou aqui para orientar!`);
        }

        giveContextualTip(state) {
            let tipText = "As coisas parecem estáveis. Continue com o bom planejamento!";
            let tipTitle = "Relatório do Professor Sabiá";
            const problemTip = SABIA_KNOWLEDGE_BASE.gameTips.find(tip => tip.condition(state));
            if (problemTip) {
                tipText = problemTip.tip;
                ...
                tipText = Helpers.getRandomItem(genericTips);
                tipTitle = "🦉 Dica do Professor Sabiá";
            }
            this.ui.updateSabiaTip(tipTitle, tipText);
        }
        ...
        attemptToAskQuiz(state, gameInstance) {
            ...
            if (state.currentMonth > this.lastQuizMonth + 1 && Math.random() < 0.3) {
                ...
                if (question) {
                    ...
                    this.ui.showSabiaQuizModal(question, (isCorrect) => { gameInstance.handleQuizResult(isCorrect); });
                }
            }
        }

        logQuizResult(isCorrect) {
            if (isCorrect) { this.log("Parabéns, resposta correta! Você ganhou +3 Pontos de P&D como bônus!", 'good'); }
            else { this.log("Quase! Mas não se preocupe, o importante é aprender. Continue estudando o código!", 'bad'); }
        }
    }
    ...
    class Game {
        ...
        advanceMonth(isFirstMonth = false) {
            ...
            if (!isFirstMonth) {
                ...
                const profitStr = summary.profit >= 0
                    ? `<span style="color:var(--color-success)">+${Helpers.formatCurrency(summary.profit)}</span>`
                    : `<span style="color:var(--color-danger)">${Helpers.formatCurrency(summary.profit)}</span>`;
                this.ui.addLog(`Mês fechado · Receita: ${Helpers.formatCurrency(summary.income)} · Custos: ${Helpers.formatCurrency(summary.expenses)} · Resultado: ${profitStr}`, 'neutral');
            } else {
            ...
            if (event) {
                ...
                this.ui.showEventModal(event, (choiceIndex) => { this.handleEventChoice(event, choiceIndex); });
        ...
        handleAction(actionId) {
            if (this.isEventActive) return;
            const action = GAME_DATA.actions[actionId]; if (!action) return;
            this.state.applyItemEffects(action);
            this.ui.addLog(`✓ Ação: <strong>${action.label}</strong> (Custo: ${Helpers.formatCurrency(action.cost || 0)})`, 'good');
            this.ui.render(this.state.get(), this.state.history, this.isEventActive);
        }

        handleResearch(techId) {
            if (this.isEventActive) return;
            const tech = GAME_DATA.researchTree[techId]; if (!tech) return;
            this.state.applyItemEffects(tech);
            this.ui.addLog(`🔬 Pesquisa: <strong>${tech.label}</strong> (Custo: ${tech.costRP} P&D)`, 'good');
            this.ui.render(this.state.get(), this.state.history, this.isEventActive);
        }

        handleEventChoice(event, choiceIndex) {
            const choice = event.choices[choiceIndex]; if (!choice) return;
            this.state.applyEventChoice(choice);
            this.state.triggerEvent(event.id);
            this.ui.addLog(`→ Resultado: ${choice.log}`, 'neutral');
            this.isEventActive = false;
            this.ui.render(this.state.get(), this.state.history, this.isEventActive);
